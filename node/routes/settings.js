...
// ============================================
// DEPARTMENTS MANAGEMENT - Admin and Dean can view, only Admin can edit
// ============================================

// Get all departments
router.get('/departments', auth, deanAuth, async (req, res) => {
    try {
...
router.post('/departments', auth, adminAuth, async (req, res) => {
    try {
        ...
        const [existing] = await db.query('SELECT id FROM departments WHERE code = ?', [code.toUpperCase()]);
...
router.delete('/departments/:id', auth, adminAuth, async (req, res) => {
    try {
        ...
        const [docs] = await db.query('SELECT COUNT(*) as count FROM documents WHERE department_id = ?', [id]);
        if (docs[0].count > 0) {
            return res.status(400).json({ 
                msg: 'Cannot delete department with existing documents. Please reassign or delete documents first.' 
            });
        }
        
        // Soft delete
        await db.query('UPDATE departments SET is_active = 0 WHERE id = ?', [id]);
